...
The gutter between columns is always `gap-2` (8px), consistent across
all breakpoints. As the viewport grows, column count increases and
column widths scale — but the gutter never changes.
...
The gutter is a structural constant, not a design variable.
Do not adjust it per component or breakpoint.
...
| Section | `gap-8` | 32px | Between major page sections (e.g. filter bar + data table, page heading + form block) |
