...
`;
  assert.deepEqual(lintWisdom(wisdom, fact), []);
});
...
`;
  const v = lintWisdom(wisdom, fact);
...
→ \`Form.Input\`
`;
  assert.deepEqual(lintKnowledge(knowledge, fact), []);
});

test('KNOWLEDGE: unknown component fails', () => {
  const fact = parseFact(FIXTURE_FACT);
  const knowledge = `→ \`Modal variant="alert"\``;
  const v = lintKnowledge(knowledge, fact);
  assert.equal(v.length, 1);
