...
Exception: `font-sejong-bold` may be used with `type-body` in docs-app navigation elements (Header logo, Header section nav, Sidebar category headings, Sidebar nav links) for brand identity. This exception applies only to `apps/docs/` navigation chrome, not to page content or `packages/web/` components. [source:header-sidebar-redesign/2026-04-04]
