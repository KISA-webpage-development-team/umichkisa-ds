...
| `type-h4` (referenced by CardTitle, undefined in source) | — | `typography.h4` once shipped | deferred | A2 deferred action item. Compiler errors loudly if `CardTitle` references a missing class — surfaces the gap. |
...
- **`type-h4` ship:** `CardTitle.tsx` references `type-h4` (currently undefined). A2 already filed this as deferred; A4 surfaces it as a *compile-error* that the compiler will emit until the class is shipped. Until shipped, the compiler will need a known-missing list to allow the build to pass. Decision deferred to impl time.
