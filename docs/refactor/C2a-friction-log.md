...
- **Actioned**: Draft must-rule `forms-formitem-aria-wiring` (Must / static) added to `A3-usage-schema.md` §"Deferred action items" so C2b picks it up. Cross-references Layer 2 invariant `formitem-htmlfor-aria-wiring` for detection logic.
